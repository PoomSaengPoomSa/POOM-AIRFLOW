...
with DAG(
    ...
    run_ai_todo_agent = BashOperator(
        ...
        bash_command='''
        curl -f -s -H "Content-Type: application/json" -X POST -d '{"u_id": "pb_b1_1", "date": "{{ logical_date.in_timezone("Asia/Seoul").strftime("%Y-%m-%d") }}"}' http://poom-alb-1963738321.ap-northeast-2.elb.amazonaws.com:8000/api/v1/ai-todo/run
        ''',
